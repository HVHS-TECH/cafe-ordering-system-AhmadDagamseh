...
});checkoutItemsEl.addEventListener(lick", function (e) {if (e.target && e.target.classList.contains("remove-btn")) {const idx = Number(e.target.dataset.index);removeItem(idx);}});function completeOrder() {if (!cart.length) {alert("Your cart is empty.");return;}let total = 0;
